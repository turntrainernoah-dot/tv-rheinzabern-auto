...
def test_regression_plan_shape():
    ...
    _FILLER = ("Aufbauen", "Abbauen", "Springer")
    for t, cells in holders.items():
        labels = {txt.replace("AW ", "") for txt, _ck in cells if txt not in _FILLER}
        check(f"{t}: genau ein Gruppen-Label", len(labels) == 1, f"{labels}")
    g1_cells = next(c for t, c in holders.items() if "G1" in {x.replace("AW ", "") for x, _ in c if x not in _FILLER})
    check("G1 Zeile0=aufwaermen", g1_cells[0][1] == "aufwaermen")
    check("G1 Zeile1=g1_blau (Geraet1)", g1_cells[1][1] == "g1_blau")
    check("G1 letzte Zeile=Abbauen (tatsaechlich letzte Zeile des Tagesrasters)",
          g1_cells[-1] == ("Abbauen", "aufbauen"), f"{g1_cells[-1]}")
...
def test_merge_compatibility():
    ...
    for c in holder_cells:
        labels = {t.replace("AW ", "") for t, ck in c if t not in ("Aufbauen", "Abbauen", "Springer")}
...
# ════════════════════════════════════════════════════════════════
# 5a2) Aufbauen/Abbauen/Springer-Sonderregel (Noah, 07.09.2026): "Aufbauen"
#      gibt es woertlich nur im festen Fenster 17:00-17:30. "Abbauen" ist
#      NICHT an eine feste Uhrzeit gebunden, sondern immer die tatsaechlich
#      LETZTE Zeile des jeweiligen Tagesrasters (endet das Training normal
#      um 19:00, ist das die 18:15-19:00-Zeile; laeuft es wegen einer
#      spaeter endenden Gruppe bis 19:30, ist es die 19:00-19:30-Zeile).
#      Jede Randzeile DAZWISCHEN heisst "Springer".
# ════════════════════════════════════════════════════════════════
def test_aufbauen_fest_abbauen_dynamisch_letzte_zeile():
    gruppen = ["G1", "G2", "G3", "G4"]
    turner = {g: [f"{g}K{i}" for i in range(1, 6)] for g in gruppen}
    ...
    for t, cells in plan.items():
        ...
        for i, (txt, _ck) in enumerate(cells):
            ...
            if txt == "Abbauen" and i != last_row:
                fehler.append(f"{t} Zeile {i}: 'Abbauen' ist nicht die letzte Zeile ({grid_rows[i]})")
    check("'Aufbauen' nur im 17:00-17:30-Fenster, 'Abbauen' nur in der letzten Zeile", not fehler, f"{fehler}")
    # T1 haelt G1+G2 (endet vor G3/G4, aber die letzte Zeile des Tagesrasters
    # richtet sich nach G3/G4) -> T1s Randzeile danach ist 'Abbauen'.
    t1_cells = plan.get("T1")
    if t1_cells:
        check("Randzeile in der tatsaechlich letzten Zeile ist 'Abbauen'",
              t1_cells[-1] == ("Abbauen", "aufbauen"), f"{t1_cells}")
...
def test_ki_path_parity():
    ...
    def unit_count(plan):
        return len({tuple(sorted({t.replace("AW ", "") for t, ck in c if t not in ("Aufbauen", "Abbauen", "Springer")}))
...
if __name__ == "__main__":
    ...
    test_aufbauen_fest_abbauen_dynamisch_letzte_zeile()
